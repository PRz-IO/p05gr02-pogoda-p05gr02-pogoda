...
def porada(pogoda): #do dokończenia
    if(pogoda.id_stan_pogody == 2 or pogoda.id_stan_pogody == 7):
        ...
        return("Zapowiada się na deszcz - weź parasol!")
    if(pogoda.id_stan_pogody == 3):
        #print("Zapowiada się na burzę!")
        return("Zapowiada się na burzę!")
    if(pogoda.id_stan_pogody == 5):
        #print("Zapowiada się na mgłę - uważaj na drodze!")
        return("Zapowiada się na mgłę - uważaj na drodze!")
    if(pogoda.poludnie > 25):
        #print("Zapowiada się na gorący dzień - pij dużo wody!")
        return("Zapowiada się na gorący dzień - pij dużo wody!")
                    ...
                    if(dzien.data.date() == datetime.today().date()):
                        ...
                        with mail.connect() as conn:
                            for uzytkownik in miasto.uzytkownicy:
                                ...
                                wiadomosc = Message("Powiadomienie o pogodzie - "+miasto.nazwa_miasta, recipients = [uzytkownik.email])
